...
void Trackr::glutIdle()
{
    glutPostRedisplay();
}
void Trackr::glutDisplay()
{
    Trackr::ms_self->Display(Trackr::ms_self->m_pUserTracker1, 0, 0, GL_WIN_SIZE_X/2, GL_WIN_SIZE_Y, false);
    Trackr::ms_self->Display(Trackr::ms_self->m_pUserTracker2, GL_WIN_SIZE_X/2, 0, GL_WIN_SIZE_X, GL_WIN_SIZE_Y, true);
}
void Trackr::glutKeyboard(unsigned char key, int x, int y)
{
    Trackr::ms_self->OnKey(key, x, y);
}

Trackr::Trackr(const char* strSampleName) : m_poseUser(0)
{
    ms_self = this;
    strncpy(m_strSampleName, strSampleName, ONI_MAX_STR);
    m_pUserTracker1 = new nite::UserTracker;
...
void Trackr::Finalize()
{
    delete m_pUserTracker1;
    delete m_pUserTracker2;
    nite::NiTE::shutdown();
    openni::OpenNI::shutdown();
}

openni::Status Trackr::Init(int argc, char **argv)
{
    m_pTexMap = NULL;
    ...
    if (m_pUserTracker1->create(&m_device1) != nite::STATUS_OK)
    {
        printf("Failed to open tracker1\n");
        return openni::STATUS_ERROR;
    }
    if (m_pUserTracker2->create(&m_device2) != nite::STATUS_OK)
    {
        printf("Failed to open tracker2\n");
        return openni::STATUS_ERROR;
    }
...
openni::Status Trackr::Run()    //Does not return
{
    glutMainLoop();
...
void DrawLimb(nite::UserTracker* pUserTracker, const nite::SkeletonJoint& joint1, const nite::SkeletonJoint& joint2, int color, int sx, int sy)
{
    ...
    coordinates[1] *= GL_WIN_SIZE_Y/(1*(float)g_nYRes);
    coordinates[3] *= GL_WIN_SIZE_X/(2*(float)g_nXRes);
    coordinates[4] *= GL_WIN_SIZE_Y/(1*(float)g_nYRes);

    coordinates[0] += sx;
    coordinates[1] += sy/(float)g_nYRes;
    coordinates[3] += sx;
    coordinates[4] += sy/(float)g_nYRes;
...
void DrawSkeleton(nite::UserTracker* pUserTracker, const nite::UserData& userData, int sx, int sy)
{
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_HEAD), userData.getSkeleton().getJoint(nite::JOINT_NECK), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER), userData.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW), userData.getId() % colorCount, sx, sy);
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW), userData.getSkeleton().getJoint(nite::JOINT_LEFT_HAND), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_RIGHT_SHOULDER), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_ELBOW), userData.getId() % colorCount, sx, sy);
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_RIGHT_ELBOW), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HAND), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_SHOULDER), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER), userData.getSkeleton().getJoint(nite::JOINT_TORSO), userData.getId() % colorCount, sx, sy);
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_RIGHT_SHOULDER), userData.getSkeleton().getJoint(nite::JOINT_TORSO), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_TORSO), userData.getSkeleton().getJoint(nite::JOINT_LEFT_HIP), userData.getId() % colorCount, sx, sy);
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_TORSO), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HIP), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_HIP), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HIP), userData.getId() % colorCount, sx, sy);


    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_HIP), userData.getSkeleton().getJoint(nite::JOINT_LEFT_KNEE), userData.getId() % colorCount, sx, sy);
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_LEFT_KNEE), userData.getSkeleton().getJoint(nite::JOINT_LEFT_FOOT), userData.getId() % colorCount, sx, sy);

    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HIP), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_KNEE), userData.getId() % colorCount, sx, sy);
    DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_RIGHT_KNEE), userData.getSkeleton().getJoint(nite::JOINT_RIGHT_FOOT), userData.getId() % colorCount, sx, sy);
}


void Trackr::Display(nite::UserTracker* tracker, int startx, int starty, int stopx, int stopy, bool swap)
{
...
//                    // Add debug lines - every 10cm
    ...
    for (int i = 0; i < users.getSize(); ++i)
    {
        ...
        else if (!user.isLost())
        {
            ...
            {
                DrawStatusLabel(tracker, user);
            }
            if (g_drawCenterOfMass)
            {
                DrawCenterOfMass(tracker, user);
            }
            ...
            {
                DrawSkeleton(tracker, user, startx, 0);
            }
        }
...
void Trackr::OnKey(unsigned char key, int /*x*/, int /*y*/)
{
    switch (key)
    {
    case 27:
...
openni::Status Trackr::InitOpenGL(int argc, char **argv)
{
    glutInit(&argc, argv);
